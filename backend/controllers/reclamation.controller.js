...
exports.createReclamation = async (req, res) => {
  try {
    const { utilisateurId, parkingId, typeReclamation, photoEvidence } = req.body;
    // Validate ObjectId format
    if (!mongoose.Types.ObjectId.isValid(utilisateurId) || !mongoose.Types.ObjectId.isValid(parkingId)) {
      return res.status(400).json({ message: 'Invalid utilisateurId or parkingId format.' });
    }
    ...
    const reclamation = new Reclamation({ utilisateurId, parkingId, typeReclamation, photoEvidence });
...
exports.getArchivedReclamations = async (req, res) => {
  try {
    const archivedReclamations = await Reclamation.find({ statut: 'Archivée' })
...
exports.updateReclamationStatus = async (req, res) => {
  try {
    ...
    // Validate statut
    const validStatuts = ['Validée', 'Archivée', 'Résolue', 'Refusée', 'En Cours'];
    ...
    } else if (updatedReclamation.statut === 'En Cours') {
